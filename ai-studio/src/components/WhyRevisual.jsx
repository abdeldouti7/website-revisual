...
const WhyRevisual = () => {
  ...
  const cards = [
    {
      ...
      artifact: (
        <div className="absolute bottom-6 right-6 w-16 h-16 bg-cream rounded-full flex items-center justify-center border border-pebbles/5 opacity-40 group-hover:opacity-100 transition-opacity">
      ...
      artifact: (
        <div className="absolute bottom-6 right-6 w-16 h-16 bg-cream rounded-full flex items-center justify-center border border-pebbles/5 opacity-40 group-hover:opacity-100 transition-opacity">
          <div className="flex items-end gap-0.5 px-2 scale-75">
            <div className="w-2 bg-silver-fern/10 rounded-t h-3"></div>
            <div className="w-2 bg-silver-fern/20 rounded-t h-4"></div>
            <div className="w-2 bg-silver-fern/40 rounded-t h-5"></div>
            <div className="w-2 bg-lemon-grass rounded-t h-6"></div>
          </div>
        </div>
      ),
    },
  ...
  return (
    <section className="py-24 px-5 lg:px-10 max-w-7xl mx-auto bg-cream">
      <div className="text-center mb-12">
        <h2 className="font-heading font-semibold text-3xl md:text-4xl text-silver-fern tracking-tight">{c.heading}</h2>
      </div>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 lg:gap-8">
        {cards.map(({ Icon, title, bullets, artifact }, i) => (
          <div key={i} className="bg-white p-6 lg:p-8 rounded-[2rem] shadow-sm border border-pebbles/5 hover-lift flex flex-col group transition-all duration-300 hover:border-lemon-grass/50 hover:shadow-md relative overflow-hidden">
            <div className="flex items-center gap-4 mb-6 relative z-10">
              <div className="w-12 h-12 bg-cream rounded-2xl flex items-center justify-center border border-pebbles/5 shrink-0">
                <Icon className="w-6 h-6 text-silver-fern" />
              </div>
              <h3 className="font-heading font-semibold text-xl text-silver-fern leading-tight">{title}</h3>
            </div>
            <ul className="space-y-4 font-sans text-pebbles/80 text-sm mt-auto relative z-10 pr-16">
              {bullets.map((b, j) => (
